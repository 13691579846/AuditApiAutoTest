...
class HandleJson(object):
    ...
    @staticmethod
    def file_json_to_dict(filename):
    ...
    @staticmethod
    def dict_to_file_json(filename, python_data):
...
if __name__ == '__main__':
    ...
    print('json格式的数据转换为python数据类型', HandleJson.json_to_dict(json_dict))
    print('json格式的数据转换为python数据类型', HandleJson.json_to_dict(json_list_dict))
    python_dict = {'code': 1, 'name': 'linux'}
    python_list_dict = ({'code': 1, 'name': 'linux'}, {'data': None, 'name': 'linux'})
    print('python类型的数据转换为json格式', HandleJson.dict_to_json(python_dict))
    print('python类型的数据转换为json格式', HandleJson.dict_to_json(python_list_dict))

    python_value = HandleJson.file_json_to_dict('json_file.txt')
    ...
    HandleJson.dict_to_file_json('python_file.txt', py_value)